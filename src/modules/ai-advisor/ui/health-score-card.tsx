    ...
    <Card className="relative overflow-hidden border-2 border-[color:var(--card-border)] bg-gradient-to-br from-[color:var(--card-bg)] to-[color:var(--color-surface-sunken)] p-5 shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[color:var(--card-border)] pb-4">
        ...
        <div className="flex items-center gap-3">
          <span className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-semibold ${badgeConfig.classes}`}>
            <span className="size-2 rounded-full bg-current animate-pulse" />
            {report.statusLabel}
          </span>
          <Link
            href="/app/diagnostico-ia"
            className="inline-flex min-h-9 items-center justify-center rounded-lg bg-[color:var(--color-brand-600)] px-3 text-xs font-semibold text-white shadow hover:bg-[color:var(--color-brand-700)] transition"
          >
            Consultor IA 🤖
          </Link>
        </div>
      ...
      <div className="mt-4 grid grid-cols-1 gap-6 md:grid-cols-12 md:items-center">
        {/* Score Gauge */}
        <div className="flex flex-col items-center justify-center rounded-xl bg-[color:var(--card-bg)] p-4 text-center md:col-span-4 border border-[color:var(--card-border)] shadow-xs">
          ...
          <div className="my-2 flex items-baseline justify-center gap-1">
            <span className={`text-4xl font-extrabold tabular ${badgeConfig.textClass}`}>
              {report.score}
            </span>
          ...
          <div className="w-full bg-[color:var(--color-ink-100)] h-2 rounded-full overflow-hidden">
            <div
          ...
          <p className="mt-2 text-xs text-balance font-medium" style={{ color: "var(--muted-fg)" }}>
            {report.summary}
          </p>
        </div>
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between gap-2">
                  ...
                  <span className={`text-xs font-bold tabular ${pillarBadge.textClass}`}>
                    {pillar.score} pts
                  </span>
                </div>
...
function getStatusBadge(status: HealthStatus): {
  ...
  switch (status) {
    case "EXCELLENT":
      return {
        classes: "bg-[color:var(--color-positive-100)] text-[color:var(--color-positive-700)] border border-[color:var(--color-positive-600)]/30",
        bgClass: "bg-[color:var(--color-positive-600)]",
        textClass: "text-[color:var(--color-positive-700)]",
      };
    case "HEALTHY":
      return {
        classes: "bg-[color:var(--color-brand-100)] text-[color:var(--color-brand-700)] border border-[color:var(--color-brand-600)]/30",
        bgClass: "bg-[color:var(--color-brand-600)]",
        textClass: "text-[color:var(--color-brand-700)]",
      };
    case "BALANCED":
      return {
        classes: "bg-blue-50 text-blue-700 dark:bg-blue-950 dark:text-blue-200 border border-blue-300",
        bgClass: "bg-blue-600",
        textClass: "text-blue-700 dark:text-blue-300",
      };
    case "ATTENTION":
      return {
        classes: "bg-[color:var(--color-attention-100)] text-[color:var(--color-attention-700)] border border-[color:var(--color-attention-600)]/30",
        bgClass: "bg-[color:var(--color-attention-600)]",
        textClass: "text-[color:var(--color-attention-700)]",
      };
    case "CRITICAL":
      return {
        classes: "bg-[color:var(--color-critical-100)] text-[color:var(--color-critical-700)] border border-[color:var(--color-critical-600)]/30",
