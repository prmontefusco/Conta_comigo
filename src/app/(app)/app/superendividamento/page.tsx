...
export default function SuperendividamentoPage() {
  ...
  // Mês inteiro: este número vira a renda declarada no dossiê jurídico, e
  // o mês corrente parcial a apresentaria menor do que é.
    ...
    try {
      const response = await fetch("/api/ai/superendividamento", {
        ...
        body: JSON.stringify({
          ...
          despesasEssenciaisDescricao: essenciais
            .map((e) => `${e.descricao} (${formatMoney(e.valorMensal)})`)
            .join(", "),
          cortesRealizadosDescricao: cortes
            .map(
              (c) =>
                `• ${c.descricao}: economia de ${formatMoney(c.economiaMensal)}/mês (${c.dataCorte ?? "cancelado"})`,
            )
            .join("\n"),
          ...
          credoresResumo: credores
            .map(
              (c) =>
                `${c.instituicao}: Saldo ${formatMoney(c.saldoDevedorEstimado)}, Parcela ${formatMoney(c.valorParcelaMensal)}`,
            )
            .join("; "),
  ...
  const adicionarDespesaEssencial = () => {
    ...
    setEssenciais((prev) => [
      ...
      {
        id: `e_${Date.now()}`,
        categoria: "OUTRO_ESSENCIAL",
        descricao: novoEssencialNome.trim(),
        valorMensal: valor,
      },
    ]);
  ...
  const adicionarGastoCortado = () => {
    ...
    setCortes((prev) => [
      ...
      {
        id: `c_${Date.now()}`,
        categoria: "OUTRO_CORTE",
        descricao: novoCorteNome.trim(),
        economiaMensal: valor,
        dataCorte: "Cortado voluntariamente",
      },
    ]);
    setNovoCorteNome("");
    setNovoCorteValor("");
          ...
          <Card className="border-l-4 border-l-[color:var(--color-critical-600)]">
            ...
            <dl className="mt-4 grid grid-cols-2 gap-4 sm:grid-cols-4">
              ...
              <Stat
                ...
                value={diagnostico.minimoExistencialCalculado}
                tone="neutral"
                hint="Protegido por lei"
              />
              <Stat
                label="Parcelas Mensais Atuais"
                value={diagnostico.totalParcelasAtuais}
                tone="critical"
              />
              <Stat
                label="Capacidade Real de Amortização"
                value={diagnostico.margemDisponivelParaPagamento}
                tone="positive"
                hint="Renda livre para o plano"
              />
            </dl>
          </Card>
          ...
          <div className="rounded-2xl border border-[color:var(--card-border)] bg-white p-6 text-neutral-900 shadow-sm print:border-none print:p-0 print:shadow-none">
            ...
            <div className="border-b-2 border-neutral-800 pb-4 text-center">
              ...
              <p className="text-xs font-semibold text-neutral-600">
                Instrumento Financeiro-Contábil fundamentado na Lei nº 14.181/2021 e no Decreto nº
                11.567/2023
              </p>
              <p className="text-2xs mt-1 text-neutral-500">
                Requerente:{" "}
                <strong>{profile?.displayName ?? household?.name ?? "Consumidor"}</strong> |
            ...
            <div className="mt-5 rounded-lg border border-neutral-200 bg-neutral-50/70 p-4">
              <div className="flex items-center justify-between">
                ...
                {(!profile?.cpf || !profile?.address?.street) && (
                  <Link
                    href="/app/meus-dados"
                    className="text-2xs rounded border border-amber-300 bg-amber-100 px-2 py-0.5 font-semibold text-amber-700 hover:underline print:hidden"
                  >
                    ⚠️ Completar CPF e Endereço em Meus Dados
                  </Link>
                )}
              </div>
              <div className="mt-3 grid grid-cols-1 gap-x-4 gap-y-2 text-xs text-neutral-700 sm:grid-cols-2">
                <div>
              ...
              {profile?.familyMembers && profile.familyMembers.length > 0 && (
                <div className="mt-4 border-t border-neutral-200 pt-3">
                  <p className="text-2xs mb-2 font-bold tracking-wider text-neutral-700 uppercase">
                  ...
                  <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                    {profile.familyMembers.map((member) => (
                      <div
                        key={member.id}
                        className="flex items-center justify-between rounded border border-neutral-200 bg-white px-3 py-1.5 text-xs text-neutral-800"
                      >
                        <div>
                          <strong className="text-neutral-900">{member.name}</strong>
                          <span className="text-2xs ml-1 text-neutral-500">
                            ({member.relationship})
                          </span>
                        </div>
                        <span
            ...
            <div className="mt-5 rounded-lg border border-neutral-200 bg-neutral-50 p-4">
              ...
              <p className="mt-1 text-justify text-xs leading-relaxed text-neutral-700">
                Declara o(a) Requerente, sob as penas da lei, que todas as dívidas constantes neste
                documento foram contraídas de boa-fé para consumo próprio e manutenção do lar,
                jamais mediante dolo, fraude ou intenção de não adimplir. A incapacidade de
                pagamento sobreveio de forma involuntária em razão de:{" "}
                <em>&quot;{motivoCrise}&quot;</em>.
              </p>
            </div>
            ...
            {narrativaIA ? (
              <div className="mt-5 rounded-lg border border-neutral-200 bg-neutral-50/50 p-4">
                <h3 className="mb-2 text-xs font-bold tracking-wider text-neutral-800 uppercase">
                  Fundamentação Fática e Jurídica da Situação Orçamentária
                </h3>
                <div className="space-y-2 text-justify text-xs leading-relaxed whitespace-pre-line text-neutral-800">
                  {narrativaIA}
                </div>
              </div>
            ) : (
              <div className="mt-4 flex items-center justify-between rounded-xl border border-[color:var(--card-border)] bg-[color:var(--color-surface-sunken)] p-3 print:hidden">
                <span className="text-xs" style={{ color: "var(--muted-fg)" }}>
                  Deseja que a Inteligência Artificial redija a fundamentação técnica para anexar à
                  petição?
                </span>
                <Button
            ...
            <div className="mt-6">
              <h3 className="border-b border-neutral-300 pb-1 text-xs font-bold tracking-wider text-neutral-900 uppercase">
                Quadro 1: Renda Líquida e Composição do Mínimo Existencial
              </h3>
              <div className="mt-2 grid grid-cols-2 gap-4 text-xs">
                <div>
                  <p>
                    <strong>Renda Mensal Líquida:</strong> {formatMoney(diagnostico.rendaLiquida)}
                  </p>
                  <p>
                    <strong>Dependentes do Núcleo Familiar:</strong> {dependentes} pessoa(s)
                  </p>
                  <p>
                    <strong>Piso Legal Mínimo Existencial:</strong>{" "}
                    {formatMoney(diagnostico.minimoExistencialCalculado)}
                  </p>
                </div>
                <div>
                  <p>
                    <strong>Total Despesas Essenciais:</strong>{" "}
                    {formatMoney(diagnostico.totalDespesasEssenciais)}
                  </p>
                  <p>
                    <strong>Parcelas de Dívidas Cobradas:</strong>{" "}
                    {formatMoney(diagnostico.totalParcelasAtuais)}
                  </p>
                  <p>
                    <strong>Saldo Disponível para Rateio:</strong>{" "}
                    {formatMoney(diagnostico.margemDisponivelParaPagamento)}
                  </p>
                </div>
              </div>

              <div className="mt-3">
                <table className="w-full border border-neutral-200 text-left text-xs">
                  <thead className="bg-neutral-100">
                    <tr>
                      <th className="border-b p-1.5">Item Essencial de Sobrevivência</th>
                      <th className="border-b p-1.5">Categoria</th>
                      <th className="border-b p-1.5 text-right">Valor Mensal</th>
                    </tr>
                  </thead>
            ...
            <div className="mt-6">
              <h3 className="border-b border-neutral-300 pb-1 text-xs font-bold tracking-wider text-neutral-900 uppercase">
                Quadro 2: Medidas Concretas de Redução de Gastos Já Adotadas pelo Consumidor
                (Boa-Fé)
              </h3>
              <p className="text-2xs mt-1 text-justify text-neutral-600">
                Demonstra-se que o requerente agiu diligentemente reduzindo ao máximo o padrão de
                vida e cancelando quaisquer serviços supérfluos, gerando uma contenção orçamentária
                comprovada de{" "}
              ...
              <div className="mt-2">
                <table className="w-full border border-neutral-200 text-left text-xs">
                  <thead className="bg-neutral-100">
                    <tr>
                      <th className="border-b p-1.5">Despesa / Serviço Cancelado</th>
                      <th className="border-b p-1.5">Status / Data</th>
                      <th className="border-b p-1.5 text-right">Economia Gerada</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-neutral-200">
                    {cortes.map((c) => (
            ...
            <div className="mt-6">
              ...
              <p className="text-2xs mt-1 text-justify text-neutral-600">
                Proposta de quitação em 60 parcelas mensais, com carência de 180 dias para início
                dos pagamentos, com rateio proporcional da capacidade financeira líquida mensal (
                {formatMoney(diagnostico.margemDisponivelParaPagamento)}):
              </p>
              <div className="mt-2">
                <table className="w-full border border-neutral-200 text-left text-xs">
                  <thead className="bg-neutral-100">
                    <tr>
                      <th className="border-b p-1.5">Instituição Credora</th>
                      <th className="border-b p-1.5 text-right">Saldo Devedor</th>
                      <th className="border-b p-1.5 text-right">% Rateio</th>
                      <th className="border-b p-1.5 text-right font-bold">Parcela Proposta</th>
                      <th className="border-b p-1.5 text-right">Total 60m</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-neutral-200">
                    {diagnostico.planoRepactuacao.credoresPropostas.map((prop) => (
                      <tr key={prop.credorId}>
                        ...
                        <td className="p-1.5 text-right">
                          {formatMoney(prop.totalAPagarEm60Meses)}
                        </td>
                      </tr>
                    ))}
                  </tbody>
            ...
            <div className="mt-6 border-t border-neutral-300 pt-4">
              ...
              <div className="text-2xs mt-2 space-y-2 text-neutral-700">
