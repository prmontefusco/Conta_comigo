...
export function Modal({
  ...
  //
  // Eram fixos — `modal-title` e `modal-description` — e uma tela com dois
  // modais montados ao mesmo tempo (a de Membros tem dois) produzia ids
  // duplicados no documento. O navegador resolve `aria-labelledby` para o
  // primeiro que encontrar, então o segundo diálogo era anunciado com o
  // título do primeiro.
  const titleId = useId();
      ...
      <div className="max-h-[85dvh] overflow-y-auto p-4 sm:p-5">
        <div className="mb-4 flex items-start justify-between gap-4">
          <div>
            ...
            {description ? (
              <p id={descriptionId} className="mt-1 text-sm" style={{ color: "var(--muted-fg)" }}>
                {description}
              </p>
            ) : null}
          </div>
